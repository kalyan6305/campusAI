...
import { Zap, Clock, Trophy, Target, ArrowLeft, Loader, Flame, Heart, Circle } from 'lucide-react';
...
const ArenaGame = ({ onExit, levelConfig }) => {
    ...
    const activateFreeze = () => {
        if (powerups.freeze > 0 && !isFrozen) {
            ...
            }, 5000); // 5 second freeze
        }
    };

    const activateFiftyFifty = () => {
        if (powerups.fiftyFifty > 0 && hiddenOptions.length === 0) {
    ...
    // Sound effects (optional, if available in browser)
    ...
    const startGame = async () => {
        ...
        try {
            // Fetch rapid-fire MCQ questions based on level
            let company = "Generic";
            if (levelConfig?.companyTags?.length > 0) {
                // Pick a random company from the level's config to simulate real-world rounds
            ...
            if (res.data && res.data.length > 0) {
                // Ensure questions are randomized
            ...
            alert("The Arena is currently closed. Failed to fetch questions. Please try again later.");
            setGameState('start');
        }
    };
    ...
    const handleAnswer = (key) => {
        if (selectedOption || gameState !== 'playing') return; // Prevent double clicks
        ...
        if (isRight) {
            ...
            const comboBonus = Math.min(newCombo * 0.1, 2); // Max 2x combo bonus
    ...
    const claimRewards = () => {
        if (earnedXP > 0) {
            ...
            // Unlock next level
            if (levelConfig?.id) {
                completeCampaignLevel(levelConfig.id);
            }
        }
        updateChallengeProgress(2); // Complete a round/mock challenge
        
        if (maxStreak >= 5) {
            awardBadge('STREAK_7'); // Using existing badges as rewards
        }
        
        if (score > 5000) {
            awardBadge('DSA_MASTER');
        }
    ...
    if (gameState === 'start') {
        return (
            <div className="w-full max-w-4xl mx-auto p-4 md:p-8 flex flex-col items-center justify-center min-h-[80vh] animate-fade-in relative">
                <button 
                    onClick={onExit}
                    className="absolute top-0 left-0 p-3 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 hover:bg-gray-50 transition-all shadow-sm"
                >
                ...
                >
                    ...
                    <div className="flex justify-center gap-6 text-sm font-bold text-gray-600">
                        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 px-4 py-2 rounded-xl shadow-sm"><Clock className="w-4 h-4 text-blue-500" /> {timeLimit}s per Question</div>
                        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 px-4 py-2 rounded-xl shadow-sm"><Heart className="w-4 h-4 text-red-500" /> 3 Lives</div>
                        <div className="flex items-center gap-2 bg-white dark:bg-gray-800 px-4 py-2 rounded-xl shadow-sm"><Flame className="w-4 h-4 text-orange-500" /> Streak Multipliers</div>
                    </div>

                    <button 
                        onClick={startGame}
                        className={`mt-12 px-12 py-6 text-white rounded-full text-2xl font-black uppercase tracking-widest shadow-xl hover:scale-105 transition-all ${isBoss ? 'bg-gradient-to-r from-red-600 to-rose-600 shadow-red-500/30' : 'bg-gradient-to-r from-indigo-600 to-purple-600 shadow-indigo-500/30'}`}
                    >
                        START MATCH
                    </button>
                </motion.div>
        ...
        return (
            <div className="w-full max-w-4xl mx-auto p-4 md:p-8 flex flex-col items-center justify-center min-h-[80vh] animate-fade-in relative">
                <motion.div 
                    ...
                    animate={{ scale: 1, opacity: 1 }}
                    className="bg-white dark:bg-gray-800 p-12 rounded-[3.5rem] shadow-2xl border border-gray-100 dark:border-gray-700 text-center space-y-10 w-full max-w-2xl"
                >
                    <div>
                        <span className="text-6xl mb-4 block">{isWin ? '🏆' : '💀'}</span>
                        <h2 className={`text-4xl font-black uppercase tracking-tight ${isWin ? 'text-indigo-600' : 'text-rose-600'}`}>
                            {isWin ? 'Match Complete!' : 'Game Over'}
                        </h2>
                        <p className="text-gray-500 font-bold mt-2 text-lg">
                            {isWin ? 'You survived the Arena.' : 'You lost all your lives.'}
                        </p>
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full py-6 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-full font-black uppercase text-lg tracking-widest hover:scale-105 transition-all shadow-xl"
                    >
                        Claim Rewards & Exit
                    </button>
                </motion.div>
        ...
        <div className={`w-full max-w-5xl mx-auto p-4 md:p-8 min-h-[85vh] flex flex-col relative animate-fade-in ${isBoss ? 'hue-rotate-[320deg]' : ''}`}>
            ...
            <header className="flex items-center justify-between mb-8 bg-white dark:bg-gray-800 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700">
                <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-2xl font-black text-orange-500 flex items-center gap-1">
                            <Flame className={`w-5 h-5 ${streak > 2 ? 'animate-pulse' : ''}`} /> {streak}
                        </span>
                    </div>
                    {combo > 1 && (
                        <>
                            ...
                            >
                                <span className="text-[10px] font-black uppercase tracking-widest text-pink-500">Combo</span>
                                <span className="text-2xl font-black text-pink-600">x{combo}</span>
                            </motion.div>
                        </>
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 bg-rose-50 dark:bg-rose-900/20 px-4 py-2 rounded-xl border border-rose-100 dark:border-rose-900/30">
                    {[...Array(3)].map((_, i) => (
                        <Heart key={i} className={`w-6 h-6 ${i < lives ? 'text-rose-500 fill-current' : 'text-gray-300 dark:text-gray-600'} transition-all`} />
                    ))}
                </div>
            </header>
            ...
            {isBoss && (
                <div className="mb-8 space-y-2">
                    <div className="flex justify-between items-end">
                        <span className="text-sm font-black uppercase tracking-tighter text-red-600 flex items-center gap-2">
                            <span className="text-xl">👹</span> BOSS HEALTH
                        </span>
                    ...
                    <div className="h-6 w-full bg-gray-100 dark:bg-gray-900 rounded-xl overflow-hidden border-2 border-red-100 dark:border-red-900/30 p-1">
                        <motion.div 
                            className="h-full bg-gradient-to-r from-red-600 via-rose-600 to-red-600 rounded-lg relative"
                            initial={{ width: '100% '}}
            ...
            <div className={`w-full h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden mb-12 shadow-inner relative ${isFrozen ? 'ring-4 ring-blue-400 ring-offset-2 dark:ring-offset-gray-900' : ''}`}>
                <motion.div 
                    className={`h-full ${isFrozen ? 'bg-blue-400' : timerColor} rounded-full`}
                    animate={{ width: `${progress}%` }}
                    transition={{ duration: 1, ease: "linear" }}
                />
                {isFrozen && (
                    <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-[8px] font-black uppercase text-blue-900 dark:text-blue-100 italic tracking-widest">TIME FROZEN</span>
                    </div>
                )}
            </div>
                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {Object.entries(currentQuestion.options || {}).map(([key, text], i) => {
                                const isSelected = selectedOption === key;
                                const showCorrect = selectedOption !== null && key === currentQuestion.correct_option;
                                const showWrong = selectedOption === key && key !== currentQuestion.correct_option;
                                ...
                                return (
                                    <button 
                                        ...
                                        className={`p-6 rounded-[2rem] border-2 text-left transition-all duration-300 flex items-center gap-6 ${btnClass}`}
                                    >
                                        <span className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl shrink-0 ${showCorrect || showWrong ? 'bg-white/20' : 'bg-gray-100 dark:bg-gray-700'}`}>
                                            {key}
                                        </span>
                        ...
                        {/* Feedback Overlay & Boss Damage Popup */}
                            ...
                            <AnimatePresence>
                                {selectedOption && (
                                    <motion.div 
                                        ...
                                        exit={{ y: -20, opacity: 0 }}
                                        className={`mt-10 p-6 rounded-3xl text-center border-2 ${isAnswerCorrect ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700'}`}
                                    >
                                        <h3 className="text-xl font-black uppercase tracking-widest mb-2">
                                            {isAnswerCorrect ? '✓ CORRECT MATCH!' : '✕ INCORRECT'}
                                        </h3>
                                        <p className="font-bold text-sm lg:text-base">{currentQuestion.explanation}</p>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            
                            {isBoss && isAnswerCorrect && selectedOption && (
                                <motion.div 
                                    initial={{ y: 0, opacity: 1, scale: 0.5 }}
                                    animate={{ y: -100, opacity: 0, scale: 2 }}
                                    className="absolute -top-40 left-1/2 -translate-x-1/2 text-5xl font-black text-red-500 italic pointer-events-none drop-shadow-lg"
                                >
                                    CRITICAL HIT!
                                </motion.div>
                            )}
            ...
            <div className="fixed bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl p-3 rounded-[2.5rem] shadow-2xl border border-white/20 z-50">
                {[
                    { id: 'freeze', icon: <Clock className="w-5 h-5" />, count: powerups.freeze, action: activateFreeze, color: 'blue' },
                    { id: 'fiftyFifty', icon: <Circle className="w-5 h-5" />, count: powerups.fiftyFifty, action: activateFiftyFifty, color: 'purple' },
                    { id: 'revive', icon: <Heart className="w-5 h-5" />, count: powerups.revive, action: activateRevive, color: 'rose' }
                ].map(pw => (
                    <button 
                        key={pw.id}
                        disabled={pw.count <= 0 || selectedOption !== null}
                        onClick={pw.action}
                        className={`group relative p-4 rounded-full transition-all ${pw.count > 0 ? `bg-${pw.color}-500 text-white hover:scale-110 shadow-lg shadow-${pw.color}-500/20` : 'bg-gray-200 dark:bg-gray-700 text-gray-400 opacity-40 grayscale cursor-not-allowed'}`}
                    >
                        {pw.icon}
                        <span className="absolute -top-1 -right-1 w-6 h-6 bg-white dark:bg-gray-900 rounded-full flex items-center justify-center text-[10px] font-black text-gray-900 dark:text-white shadow-sm border border-gray-100 dark:border-gray-800">
                            {pw.count}
                        </span>
                    </button>
                ))  }
            ...
            <button 
                onClick={endGame}
                className="absolute top-8 right-8 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-rose-500 transition-colors"
            >
                Surrender
            </button>
        </div>
